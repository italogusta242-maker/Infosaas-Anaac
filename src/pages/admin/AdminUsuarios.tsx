...
const AdminUsuarios = () => {
  ...
  const handleStatusToggle = async (userId: string, currentStatus: string) => {
    ...
    // Optimistic update
    setAlunos((prev) => prev.map((a) => a.id === userId ? { ...a, status: newStatus } : a));
    try {
      ...
      // Revert on error
      setAlunos((prev) => prev.map((a) => a.id === userId ? { ...a, status: currentStatus } : a));
      ...
      <Tabs defaultValue="alunos" className="w-full" onValueChange={setActiveTab}>
        {!specialistFilter && (
          <TabsList className="grid w-full grid-cols-2 max-w-[400px]">
            ...
            <TabsTrigger value="profissionais">Não Alunos</TabsTrigger>
          </TabsList>
        )}
                      ...
                      <tr key={user.id} className="border-b border-border/50 hover:bg-secondary/30 transition-colors">
                        ...
                        <td className="p-3">
                          <button
                            ...
                            title={user.status === "ativo" ? "Clique para desativar" : "Clique para ativar"}
                            className="flex items-center gap-2 group"
                          >
                            {togglingStatus === user.id ? (
                              <Loader2 size={12} className="animate-spin text-muted-foreground" />
                            ) : (
                              <span className={`w-2.5 h-2.5 rounded-full transition-colors ${user.status === "ativo" ? "bg-emerald-500" : "bg-destructive"}`} />
                            )}
                            <span className={`text-xs font-medium group-hover:underline transition-colors ${user.status === "ativo" ? "text-emerald-500" : "text-destructive"}`}>
                              {user.status === "ativo" ? "Ativo" : user.status === "inativo" ? "Inativo" : user.status}
                            </span>
                          </button>
                        </td>
                        <td className="p-3">
