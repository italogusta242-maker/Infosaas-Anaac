...
const AdminDashboard = () => {
  ...
  // ── Engagement KPI (avg completion rate of active plans last 7 days) ──
  const engagedUsers = studentProfiles.filter((p) => {
    const lastW = userLastWorkout.get(p.id);
    if (!lastW) return false;
    return differenceInDays(today, parseISO(lastW)) <= 7;
  });
  ...
  const engagementData = Array.from({ length: chartDays === 1 ? 24 : chartDays }, (_, i) => {
    if (chartDays === 1) {
      ...
      return { day: `${hour.getHours()}h`, treinos: new Set(dayWorkouts.map((w) => w.user_id)).size };
    }
  ...
  const getModalDescription = (): string => {
    switch (activeModal) {
      ...
      case "engajamento": return `${engagedUsers.length} de ${studentProfiles.length} alunos treinaram nos últimos 7 dias`;
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={engagementData}>
                ...
                <Line type="monotone" dataKey="checkins" stroke="hsl(342 100% 77%)" strokeWidth={2} dot={{ fill: "hsl(342 100% 77%)", r: 2.5 }} name="Check-ins" />
              </LineChart>
            </ResponsiveContainer>
            <div className="flex gap-4 mt-2 text-xs text-muted-foreground">
              ...
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full" style={{ background: "hsl(342 100% 77%)" }} /> Check-ins</span>
            </div>
          </CardContent>
        </Card>
      </div>
