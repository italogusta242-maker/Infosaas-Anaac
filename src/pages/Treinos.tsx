...
import {
  ...
  ClipboardList, Target, TrendingUp, FileText, Share2, Navigation, Plus
} from "lucide-react";
...
// ─── Running Module ──────────────────────────────────────────
const RUNNING_TYPES = [
  { id: "Tiro", label: "Tiro", desc: "Intervalado intenso" },
  { id: "Longo", label: "Longo", desc: "Ritmo contínuo leve" },
  { id: "Recuperação", label: "Recuperação", desc: "Ritmo suave" },
];

const WEEKLY_KM_GOAL = 15;
...
const RunningModule = ({ workoutHistory, user, queryClient }: {
  workoutHistory: WorkoutLog[];
  user: any;
  queryClient: any;
}) => {
  const [logOpen, setLogOpen] = useState(false);
  const [runType, setRunType] = useState("Longo");
  const [distance, setDistance] = useState("");
  const [duration, setDuration] = useState("");

  const weekStart = getWeekStart();

  const runSessions = workoutHistory.filter(
    (w) => w.group_name?.startsWith("Corrida") && w.finished_at
  );

  const thisWeekKm = runSessions
    .filter((w) => new Date(w.started_at) >= weekStart)
    .reduce((sum, w) => {
      const km = (w as any).comment?.match(/(\d+\.?\d*)\s*km/)?.[1];
      return sum + (km ? parseFloat(km) : 0);
    }, 0);

  const totalKm = runSessions.reduce((sum, w) => {
    const km = (w as any).comment?.match(/(\d+\.?\d*)\s*km/)?.[1];
    return sum + (km ? parseFloat(km) : 0);
  }, 0);

  const pct = Math.min(100, (thisWeekKm / WEEKLY_KM_GOAL) * 100);

  const saveRun = useMutation({
    mutationFn: async () => {
      if (!user || !distance) return;
      const km = parseFloat(distance);
      if (isNaN(km) || km <= 0) throw new Error("Distância inválida");
      ...
      const { error } = await supabase.from("workouts").insert({
        user_id: user.id,
        group_name: `Corrida — ${runType}`,
        duration_seconds: durationSecs,
        comment: `${km}km — ${runType}`,
        started_at: now,
        finished_at: now,
        exercises: [],
      });
    ...
    <div className="space-y-4 mb-6">
      ...
      <div className="bg-card border border-border rounded-2xl p-5">
        <div className="flex items-center gap-4 mb-5">
          ...
          <div className="flex-1">
            <h3 className="font-sans font-bold text-foreground text-base tracking-tight">Plano de Corrida</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Cardio e Performance</p>
          </div>
        </div>
        ...
        <div className="mb-4 space-y-2">
          <div className="flex items-end justify-between">
            <div>
              <span className="text-2xl font-black text-foreground tabular-nums">{thisWeekKm.toFixed(1)}</span>
              <span className="text-sm text-muted-foreground ml-1">/ {WEEKLY_KM_GOAL} km esta semana</span>
            </div>
        ...
        <div className="flex items-center gap-4 text-center mb-5">
          <div className="flex-1 bg-secondary/50 rounded-xl p-3">
            <p className="text-xl font-black text-foreground">{runSessions.length}</p>
            <p className="text-[10px] text-muted-foreground uppercase tracking-wider">sessões</p>
          </div>
          <div className="flex-1 bg-secondary/50 rounded-xl p-3">
            <p className="text-xl font-black text-foreground">{totalKm.toFixed(0)}</p>
      ...
      <div className="space-y-2">
        {RUNNING_TYPES.map((rt) => {
          const count = runSessions.filter((w) => w.group_name?.includes(rt.id)).length;
          return (
            <div key={rt.id} className="bg-card border border-border rounded-xl p-4 flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-foreground tracking-tight mb-0.5">{rt.label}</p>
                <p className="text-xs text-muted-foreground">{rt.desc} · {count} sessões</p>
              </div>
              <button
                onClick={() => { setRunType(rt.id); setLogOpen(true); }}
                className="w-10 h-10 rounded-full bg-secondary border border-border flex items-center justify-center hover:border-accent/50 hover:bg-accent/5 transition-colors"
              >
      ...
      {runSessions.length > 0 && (
        <div className="space-y-2">
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground px-1">Histórico de Corridas</p>
          {runSessions.slice(0, 5).map((w) => {
            const km = (w as any).comment?.match(/(\d+\.?\d*)\s*km/)?.[1] || "—";
            ...
            return (
              <div key={w.id} className="bg-secondary/30 border border-border/50 rounded-xl p-3 flex items-center justify-between">
                <div>
                  <p className="text-xs font-bold text-foreground">{w.group_name?.replace("Corrida — ", "") || "Corrida"}</p>
                  <p className="text-[10px] text-muted-foreground">{date.toLocaleDateString("pt-BR", { day: "2-digit", month: "short" })}</p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-black text-accent">{km} km</p>
                  {w.duration_seconds ? (
                    <p className="text-[10px] text-muted-foreground">{formatTime(w.duration_seconds)}</p>
                  ) : null}
                </div>
              </div>
            );
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end">
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2">
              {RUNNING_TYPES.map((rt) => (
                <button
                  ...
                  className={`py-2.5 rounded-xl text-xs font-bold border transition-all ${runType === rt.id ? "bg-accent text-white border-accent shadow" : "bg-secondary/50 text-muted-foreground border-border hover:border-accent/50"}`}
                >
                  {rt.label}
                </button>
              ))}
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Distância (km)</label>
              <Input
                type="number"
                step="0.1"
                min="0"
            ...
            {/* Duration */}
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Duração (minutos)</label>
              <Input
                type="number"
                step="1"
                min="0"
            ...
            <button
              ...
              disabled={!distance || saveRun.isPending}
              className="w-full py-4 bg-accent text-white font-cinzel font-bold text-sm rounded-xl tracking-wider shadow-lg shadow-accent/20 disabled:opacity-40 hover:bg-accent/90 transition-colors"
            >
              {saveRun.isPending ? <Loader2 className="animate-spin mx-auto" size={18} /> : "Salvar Corrida"}
            </button>
          </motion.div>
        </div>
      )}
...
const Treinos = () => {
  ...
  const [activeTab, setActiveTab] = useState<"treino" | "corrida">("treino");
      ...
      <div className="p-4 max-w-lg mx-auto pb-24">
        ...
        <div className="flex bg-secondary/50 p-1 rounded-xl mb-6">
          <button
            ...
            className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors ${
              activeTab === "treino" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
            }`}
          >
            Treino
          </button>
          <button
            ...
            className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors ${
              activeTab === "corrida" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
            }`}
          >
            Corrida
          </button>
        ...
        {!plan && (
          ...
          >
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => navigate("/aluno/desafio")}
              className="mt-4 px-8 py-3 bg-foreground text-background font-sans font-bold text-sm rounded-xl tracking-wide uppercase"
            >
              Escolha o plano
            </motion.button>
          </motion.div>
        ...
        {plan && activeTab === "treino" && <div className="space-y-4 mb-4">
            ...
            {workoutGroups.map((group, i) => {
              ...
              return (
                <motion.button
                  ...
                  onClick={() => openGroup(i)}
                  className="w-full flex items-center gap-4 p-4 rounded-2xl bg-card border border-border hover:border-primary/50 transition-all relative shadow-sm"
                >
                  {i === nextGroupIndex && (
                    <span className="absolute -top-2 right-4 text-[10px] font-sans font-bold bg-primary text-primary-foreground px-2.5 py-0.5 rounded-full shadow-sm">
                      Hoje
                    </span>
                  )}
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${
                    i === nextGroupIndex ? "bg-primary/20 text-primary" : "bg-secondary text-muted-foreground"
                  }`}>
                    <Dumbbell size={20} />
                  </div>
                  <div className="flex-1 text-left">
                    <p className="text-sm font-bold text-foreground tracking-tight">{group.name}</p>
                  ...
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                    i === nextGroupIndex ? "bg-primary text-primary-foreground shadow-sm" : "bg-secondary/80 border border-border text-muted-foreground"
                  }`}>
          ...
          <div className="mt-5 pt-4 border-t border-border/50">
            <div className="flex items-center justify-between mb-2">
               <div>
                  <span className="text-xs font-semibold text-foreground">{sessionsCompleted}</span>
                  <span className="text-xs text-muted-foreground">/{totalSessions} sessões</span>
               </div>
              {plan?.valid_until && (
                <span className="text-[10px] uppercase font-bold text-muted-foreground">Válido até {new Date(plan.valid_until).toLocaleDateString("pt-BR")}</span>
              )}
            </div>
            <Progress value={totalSessions > 0 ? (sessionsCompleted / totalSessions) * 100 : 0} className="h-1.5" />
          </div>
        </div>}

        {/* Módulo de Corrida (Running Plan) */}
        {plan && activeTab === "corrida" && <RunningModule workoutHistory={workoutHistory} user={user} queryClient={queryClient} />}
