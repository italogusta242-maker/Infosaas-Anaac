...
export const useRealPerformance = () => {
  ...
  const buildPerformanceData = useCallback((numDays: number): DayPerformance[] => {
    ...
    for (let i = numDays - 1; i >= 0; i--) {
      ...
      // Planner score: 20pts max — based on daily check-in mood + notes
      // A check-in existing = 10pts, having both mood and sleep_hours = full 20pts
      let plannerPts = 0;
      if (dayCheckin) {
        plannerPts += 10;
        if (dayCheckin.sleep_hours && dayCheckin.mood) plannerPts += 10;
      }

      // Composite score: Treino(40) + Dieta(40) + Planner(20) = 100
      const score = Math.min(100, training.score + dietPts + plannerPts);
    ...
    return days;
  }, [last30Workouts, last30Checkins, habitsRange, last30Workouts, last30Checkins]); // Added all deps
