  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of files) {
        ...
        const lessonId = `item-${Date.now()}-${Math.random()}`;
        ...
        newLessons.push({
          id: lessonId,
          title: lessonTitle,
          sort_order: newLessons.length + 1,
      ...
      toast.success("PDFs enviados e processados com sucesso!");
    } catch (err: any) {
      toast.error(err.message || "Falha ao enviar e processar PDF.");
    } finally {
          ...
          <div className="space-y-4 pt-6 border-t border-border/20">
            <div className="flex items-center justify-between">
              ...
              {(['workouts', 'diets', 'running', 'planner'].includes(module.type)) ? (
                ...
                <Button
                  ...
                  onClick={() => {
                    const newItems = [...(module.lessons || []), { id: `item-${Date.now()}`, title: "Nova Aula", duration: "05:00", sort_order: (module.lessons?.length || 0) + 1 }];
            ...
            {isUploading && (
              <div className="bg-muted/5 border border-dashed border-border/40 rounded-2xl p-4 flex flex-col items-center justify-center gap-2">
                ...
                <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Processando arquivo(s)...</p>
              </div>
            )}
            ...
            <div className="space-y-2">
              {(!module.lessons || module.lessons.length === 0) && !isUploading ? (
                ...
                module.lessons?.sort((a: any, b: any) => a.sort_order - b.sort_order).map((lesson: any, idx: number) => (
